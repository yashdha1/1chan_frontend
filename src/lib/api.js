...
client.interceptors.response.use(
	...
	async (err) => {
		...
		try {
			if (!refreshing) {
				refreshing = client.post("/free_auth/refresh").finally(() => {
					refreshing = null;
				});
			}
			await refreshing;
...
export const api = {
	login: (username, password) =>
		apiFetch("/free_auth/login", {
			method: "POST",
			body: { username, password },
		}),
	register: (payload) =>
		apiFetch("/free_auth/register", { method: "POST", body: payload }),
	refresh: () => apiFetch("/free_auth/refresh", { method: "POST" }),
	...
	updateProfile: (data) =>
		apiFetch("/auth/profile", { method: "PATCH", body: data }).then(
			(r) => r?.user ?? r,
		),
	deleteOwnProfile: () => apiFetch("/auth/profile", { method: "DELETE" }),

	getUsers: (role = "all") => apiFetch(`/auth/admin/users/${role}`),
	...
	generateFeed: (
		feed_type = "suggested", // fetching the feed /home page pr :
	...
	unlikePost: (id) => apiFetch(`/posts/${id}/unlike`, { method: "POST" }),
	getPostLikedBy: (id) => apiFetch(`/posts/${id}/liked_by`, { method: "GET" }),
	...
	likeComment: (commentId) =>
		apiFetch("/posts/comments/like", {
			method: "POST",
			body: { comment_id: commentId },
		}),
	unlikeComment: (commentId) =>
		apiFetch("/posts/comments/unlike", {
			method: "POST",
			body: { comment_id: commentId },
		}),

	getTags: () => apiFetch("/feed/operation/get_tags"),
	recordViewed: (body) =>
		apiFetch("/feed/operation/viewed_posts", { method: "POST", body }),
	updateWeights: (action) =>
		apiFetch("/feed/operation/update_weights", {
			method: "POST",
			body: action,
		}),
	addTag: (tag) =>
