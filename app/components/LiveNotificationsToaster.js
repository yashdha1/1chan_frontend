...
function getNotifWsUrl() {
	...
	// WebSocket upgrades bypass Next.js HTTP rewrites, so connect to Kong directly.
	const protocol =
		globalThis.window.location.protocol === "https:" ? "wss" : "ws";
	const host = globalThis.window.location.hostname;
	return `${protocol}://${host}:8000/api/v1/notifications/ws/live`;
}
...
function toMessage(n) {
	...
	if (n.type === "mention")
		return `${actor} mentioned you. Check /notifications for details.`;
	if (n.type === "reply")
