...
export default function PostCard({
	...
	function castVote(dir) {
		setVote((prev) => {
			...
			const newScore = removing
				? post.likes
				: post.likes + (dir === "up" ? 1 : -1);
			setScore(newScore);
			if (removing) {
							...
							{likedByLoading ? (
								...
								<ul className="max-h-40 overflow-y-auto">
									{likedBy.map((u) => (
										<li
											key={u.user_name ?? u}
											className="px-3 py-0.5 text-[11px] text-zinc-300"
										>
											@{u.user_name ?? u}
										</li>
									))}
			...
			<div className="min-w-0 flex-1">
				...
				{post.imageLink && !imgError && (
					<Link href={`/posts/${post.id}`} className="mt-2 block">
						<div className="relative flex max-h-[32rem] w-full items-center justify-center overflow-hidden rounded-md bg-zinc-950/70">
							{!imgLoaded && !imgError && (
