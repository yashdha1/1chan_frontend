...
export default function EditProfilePage() {
  ...
  useEffect(() => {
    ...
    api.getProfile(currentUser.username)
      .then((p) => setForm({ username: p.username ?? "", bio: p.bio ?? "", avatar: p.avatar ?? "" }))
      .catch(() => {});
    ...
    <div className="mx-auto max-w-lg px-4 py-10">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div className="flex flex-col gap-1.5">
          <label htmlFor="edit-username" className="text-xs text-zinc-500">Username</label>
          <input
            id="edit-username"
            type="text"
            name="username"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          />
          <span className="text-right text-xs text-zinc-600">{form.bio.length}/160</span>
        </div>
