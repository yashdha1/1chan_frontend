...
export default function SignupPage() {
  ...
  const {
    step, username, password, avatarLink, bio, errors,
    isLoading, submitError,
    setUsername, setPassword, setBio, setStep,
    goToStep2, setAvatarFromFile, clearAvatarLink, register, reset,
  } = useSignupForm();
      ...
      <div className="flex flex-col gap-8">
        <header>
          ...
          <div className="mt-4 flex gap-1.5" aria-hidden>
            <span className={`h-0.5 flex-1 rounded-full ${step >= 1 ? "bg-zinc-200" : "bg-zinc-800"}`} />
            <span className={`h-0.5 flex-1 rounded-full ${step >= 2 ? "bg-zinc-200" : "bg-zinc-800"}`} />
          </div>
        </header>
          ...
          {step === 1 ? (
            <>
              <div className="flex flex-col gap-1.5">
                <label htmlFor="signup-username" className="text-xs text-zinc-500">Username</label>
                <input
                  id="signup-username"
                  type="text"
            ...
            <>
              <div className="flex flex-col items-center gap-3">
                <button
                  ...
                  style={avatarLink ? { backgroundImage: `url(${avatarLink})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined}
                >
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="text-xs text-zinc-400 hover:text-zinc-200 hover:underline underline-offset-4">
                    Upload
                  </button>
                  {avatarLink && (
                    <button type="button" onClick={() => { clearAvatarLink(); if (fileInputRef.current) fileInputRef.current.value = ""; }} className="text-xs text-zinc-600 hover:text-zinc-400">
                      Remove
                    </button>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                />
                <span className="text-right text-xs text-zinc-600">{bio.length}/160</span>
              </div>

              {submitError && <p className="text-xs text-red-400">{submitError}</p>}

              <button type="submit" disabled={isLoading} className={`${AUTH_PRIMARY_BUTTON_CLASS} disabled:opacity-60 disabled:cursor-not-allowed`}>
                {isLoading ? "Creating..." : "Create account"}
              </button>
              <button type="button" onClick={() => setStep(1)} className="w-full py-2 text-sm text-zinc-500 hover:text-zinc-300 transition-colors">
                Back
              </button>
            </>
          )}
