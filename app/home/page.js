...
import { ChevronDown, Image, Plus, Tag, X } from "lucide-react";
...
export default function HomePage() {
	...
	const [adminSearch, _setAdminSearch] = useState("");
	const [_adminSearchOpen, setAdminSearchOpen] = useState(false);
	...
	useEffect(() => {
		function handleClick(e) {
			if (
				...
				!adminSearchRef.current.contains(e.target)
			) {
	...
	const _hasAdminResults = searchedPosts.length > 0;
								...
								{selectedTags.length < MAX_TAGS && (
									<div className="relative">
										<input
											...
											onBlur={() =>
												setTimeout(() => setTagDropOpen(false), 150)
											}
